...
tool_registry.register_tool(tool=calculator)
enhanced_agent = SimpleAgent(name="增强助手", llm=llm, system_prompt="你是一个智能助手, 可以使用工具来帮助用户。", tool_registry=tool_registry, enable_tool_calling=True)
...
for chunk in basic_agent.stream_run("请解释什么是人工智能"):
    pass # 内容已经在stream_run里实时打印

# 测试4: 动态添加工具
print("=== 测试4: 动态工具管理 ===")
print(f"添加工具前: {basic_agent.has_tools()}")
basic_agent.add_tool(tool=calculator)
